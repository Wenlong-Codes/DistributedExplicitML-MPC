...
def LoadOpenLoopData():
    data_list = np.load('../0 Open-loop Simulation/e_k1_data.npy')
    dev_data_list = [] #[x_k_1, x_k_2, u_k_1, u_k_2, e_k1_1, e_k1_2]
    for item in data_list:
        x_k_1 = (item[0] - 0.832637)*100
        x_k_2 = item[1] - 199.343
        u_k_1 = item[2] - 120
        u_k_2 = item[3] - 3.5
        e_k1_1 = abs(item[4] - 0.832637)*100 #absolute value of xk1_1
        e_k1_2 = abs(item[5] - 199.343) #absolute value of xk1_2
        dev_data_list.append([x_k_1, x_k_2, u_k_1, u_k_2, e_k1_1, e_k1_2])
    return np.array(dev_data_list)

def ProcessSimData(trj_data):
    x_k_u_k_list = trj_data[:, :4]
    e_k1_list = trj_data[:, 4:]
    x_k_u_k_scaler = preprocessing.MinMaxScaler(feature_range=(-1, 1)).fit(x_k_u_k_list)
    e_k1_scaler = preprocessing.MinMaxScaler(feature_range=(0, 1)).fit(e_k1_list)
    np.save('./PICNN_scaler/x_k_u_k_scaler.npy', x_k_u_k_scaler)
    np.save('./PICNN_scaler/e_k1_scaler.npy', e_k1_scaler)
    x_k_u_k_list_scaled = x_k_u_k_scaler.transform(x_k_u_k_list)
    e_k1_list_scaled = e_k1_scaler.transform(e_k1_list)
    trj_data_scaled = np.column_stack((x_k_u_k_list_scaled, e_k1_list_scaled))
...
class SimDataset(Dataset):
    def __init__(self, trj_data):
        ...
        self.x_k = trj_data[:,:,:2]
        self.u_k = trj_data[:,:,2:4]
        self.e_k1 = trj_data[:,:,4:]

    def __len__(self):
        return int(len(self.x_k))

    def __getitem__(self, index):
        return self.x_k[index], self.u_k[index], self.e_k1[index]
    
class PICNN_Model_PH1(nn.Module):
    def __init__(self):
        super(PICNN_Model_PH1, self).__init__()
        input_dim_x = 2 #non-convex input dim    
        input_dim_u = 2 #convex input dim
        output_dim_h = 2 #output dim: e_k1
        
        self.W1h_u = nn.Linear(8, 16)
        self.W2h_u = nn.Linear(16, 32)
        self.W3h_u = nn.Linear(32, 8)
        self.W4h_u = nn.Linear(8, output_dim_h)
        self.W0u = nn.Linear(input_dim_u, 8)
        self.W1u = nn.Linear(input_dim_u, 16)
        self.W2u = nn.Linear(input_dim_u, 32)
        self.W3u = nn.Linear(input_dim_u, 8)
        self.W4u = nn.Linear(input_dim_u, output_dim_h)
        self.W0h_x = nn.Linear(input_dim_x, 8)
        self.W1h_x = nn.Linear(8, 16)
        self.W2h_x = nn.Linear(16, 32)
        self.W3h_x = nn.Linear(32, 8)
        self.W1xu = nn.Linear(8, 16)     
        self.W2xu = nn.Linear(16, 32)     
        self.W3xu = nn.Linear(32, 8)     
        self.W4xu = nn.Linear(8, output_dim_h) 
        self.relu = nn.ReLU()
...
def model_evaluation(model, loss_fn, dl):
    output_list = []
    model.eval()
    for i, (x_k, u_k, e_k1) in enumerate(dl):
        x_k = x_k.to(torch.float32).to(device)
        u_k = u_k.to(torch.float32).to(device)
        e_k1 = e_k1.to(torch.float32).to(device)
        e_k1_pred = model(x_k, u_k)
        loss = loss_fn(e_k1, e_k1_pred)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
if __name__ == "__main__":
    ...
    #ML model setup
    model = PICNN_Model_PH1().to(device)
    ...
    for epoch in range(epochs):
        model.train()
        output_list = []
        for i, (x_k, u_k, e_k1) in enumerate(tqdm(train_dl)):
            x_k = x_k.to(torch.float32).to(device)
            u_k = u_k.to(torch.float32).to(device)
            e_k1 = e_k1.to(torch.float32).to(device)
            e_k1_pred = model(x_k, u_k)
    ...
    torch.save(model, 'PICNN_e_k1.pkl')
    plot_loss(loss_list)
